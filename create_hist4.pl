...
$num_points = @points;
$end = $num_points;
print ("completed in $timeinterval \bsec\n$inputfile has ".@lines." lines and ".@points." of those are vertices\n");
print Olog ("completed in $timeinterval \bsec\n$inputfile ".@lines." lines and ".@points." of those are vertices\n");
...
while($rad < 2*$pi){
	...
	foreach $edges (@new_edges){print Mem ($edges);}
	@sheet = ();
	$x = 0;
	$il = @new_edges;
