...
$pi = 3.141592654;
$rotate = $pi/18; #sets the resolution of the rotations, default $pi/72
...
while($rad < 2*$pi){
	...
	while($index_y < $sheetsize){
		...
		while($index_x < $sheetsize ){
			...
			if ( ($first == 1) and ($temp1 == 1) and ($temp2 == 0) ) {# ($first == 1) should be possible to remove this code
				...
				push(@new_edges,"$angle_rad $x $y");#The angle is from the center of the sheet
				#print debug ("Edge found ". @new_edges. "\n");
				$last = 1;
			}
	...
	$sheet[$index] = 0;
	print debug ("Sheet size is ".@sheet);
	@new_edges = sort {$a <=> $b} @new_edges;
	@edges = (@edges,@new_edges);
	@sheet = ();
	print debug ("becomes ".@sheet."\n");
	#print debug (@new_edges." new_edges from sheet\n");
	$x = 0;
	while($x < @new_edges){
		#print debug ("$x $new_edges[$x]\n");
		$x++;
	}
	#print debug ("\n\n");
	$x = 0;
...
while($rad < 2*$pi){
	...
	while ($index < $end) {#??? I don't think so.... Now I'm not sure... might be. Works now
		($vertex, $x1, $y1, $z1) = split(/ /, $points[$index++]); 
		$y1 = ($y1*$SI + $z1*$CS);	#rotate points ( only the y points are needed for the calulation) ($x1, $y1) = ( ($y1*$CS - $z1*$SI) , (($y1*$SI + $z1*$CS) ) <- equ. for y-z plane
		($x2, $y1) = (( ($sheetsize/2) + ($resolution*$x2) ) , ( ($sheetsize/2) + ($resolution*$y1) ));#! change ($resolution*$x2) to ($resolution*$x1) to stop rotation
		($x2, $y1) = (int($x2), int($y1));
		if (($x2 < 0) or ($y1 < 0)){die("negative coordinate in sheet!!!\n")};
		$i = $sheetsize*$y1 + $x2;
...
while ($index < @edges){
	...
	#print debug ("Dis= $distance Curve= $curvature[$index]\n");
}
$x = 0;
$il = @edges;
...
print("Completed/nThe time to process $inputfile and create $outF was $hours:$min:$sec \n");
print(Olog "Completed/nThe time to process $inputfile and create $outF was $hours:$min:$sec \n");
}
print ("\a");
